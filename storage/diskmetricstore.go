...
func (dms *DiskMetricStore) processWriteRequest(wr WriteRequest) {
	...
	if wr.MetricFamilies == nil || wr.Replace {
		// No MetricFamilies means delete request. In that case, or if a
		// replace has been requested, we need to delete the
		// metricGroup. In the former case, we are also done here.
...
func (dms *DiskMetricStore) checkWriteRequest(wr WriteRequest) bool {
	...
	if timestampsPresent(wr.MetricFamilies) {
		err = errors.New("pushed metrics must not have timestamps")
		return false
	}
